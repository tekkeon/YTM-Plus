...
import { getSongInfo, getPlayerState } from './lib';
...
const initializeChangeEmitters = () => {
  ...
  titleElement &&
    songInfoObserver.observe(titleElement, { attributeFilter: ['title'] });
  ...
  playPauseElement &&
    playerStateObserver.observe(playPauseElement, {
  ...
  volumeElement &&
    playerStateObserver.observe(volumeElement, { attributeFilter: ['value'] });
...
const tryScrobble = () => {
  try {
    ...
    if (time) {
      var curTimeSec =
        parseInt(time[0].split(':')[0]) * 60 + parseInt(time[0].split(':')[1]);
      var endTimeSec =
        parseInt(time[1].split(':')[0]) * 60 + parseInt(time[1].split(':')[1]);
