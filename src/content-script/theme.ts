...
const updateTheme = () => {
  storage.get("options").then((options: Options) => {
    ...
    if (options?.ytmTheme?.logo || options?.ytmTheme?.logoText) {
      var logoEls = document.querySelectorAll("picture.ytmusic-nav-bar");

      if (!logoEls?.length) {
        logoEls = document.querySelectorAll("#custom-logo");
      }

      const logoParent = logoEls[0]?.parentElement;
