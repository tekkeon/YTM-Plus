...
import { Message, Options, YTMTheme } from '../types';
import { storage } from '../util/chrome';
import { createThemeCSS } from '../util/youtube';
...
const initializeTheme = () => {
  ...
  chrome.runtime.onMessage.addListener((message: Message, sender, sendResponse) => {
    switch(message.type) {
      case MessageType.YTM_THEME_UPDATED:
        updateTheme();
        break;
    }
  })
...
const updateTheme = () => {
  storage.get('options')
    .then((options: Options) => {
      var style = document.getElementById('ytm-custom-theme');
      const css = createThemeCSS(options?.ytmTheme);

      if (!style) {
        style = document.createElement('style');
        style.id = 'ytm-custom-theme';
      } else {
